#include <WiFi.h>

// 💡 Wi-Fi 정보
const char* ssid = "asia-edu_2G";
const char* password = "12345678";

// 🌟 PC(Python 서버) 정보 설정
const char* serverIP = "192.168.0.110"; 
const uint16_t serverPort = 10000;

WiFiClient client;

// 🛡️ [추가] 정지 명령 중복 전송 방지용 플래그
bool stopCommandSent = false; 

void setup() {
  Serial.begin(115200); 
  Serial2.begin(9600, SERIAL_8N1, 16, 17); // 아두이노 우노와 통신

  // Wi-Fi 연결
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) {
    delay(500);
    Serial.print(".");
  }
  Serial.println("\n✅ Wi-Fi 연결 성공!");

// 🌟 [IP 확인용 코드 추가] 공유기가 할당해준 내 IP 주소를 화면에 출력!
  Serial.print("🎯 내 로봇의 IP 주소는: ");
  Serial.println(WiFi.localIP());
}
void loop() {
  // 1. 서버(PC) 소켓 연결 또는 Wi-Fi 연결 상태 확인
  if (!client.connected() || WiFi.status() != WL_CONNECTED) {
    
    // 🛡️ [예외처리] 통신 단절 감지 시, 우노에게 즉각 정지 명령 하달 (1회만 전송)
    if (!stopCommandSent) {
      Serial2.println("a+0,d+0"); 
      Serial.println("🚨 [비상] 소켓/Wi-Fi 통신 단절! 아두이노 강제 정지 신호 전송");
      stopCommandSent = true; 
    }

    // 재접속 시도
    // if (client.connect(serverIP, serverPort)) {
    //   Serial.println("\n✅ PC 관제 센터와 다시 연결되었습니다!");
    //   stopCommandSent = false; // 연결 성공 시 플래그 리셋 (다음 단절을 대비)
    if (client.connect(serverIP, serverPort)) {
      Serial.println("\n✅ PC 관제 센터와 다시 연결되었습니다!");

      client.println("ID:0");   // ✅ 추가: 나는 robot0 이라고 서버에 알림
      Serial.println("📡 Robot ID 전송: ID:0");

      stopCommandSent = false;
    } else {
      delay(2000); // 2초 대기 후 다시 재접속 시도
      return;
    }
  }

  // 2. 🌟 [PC -> ESP32 -> 아두이노] 모니터링 및 배달
  if (client.available()) {
    // 줄바꿈(\n)이 올 때까지 명령어 한 줄을 통째로 읽습니다.
    String cmdFromPC = client.readStringUntil('\n');
    
    // 1) 시리얼 모니터에 "나 이거 받았어!"라고 명확히 출력
    Serial.print("👉 [FROM PC]: ");
    Serial.println(cmdFromPC); 

    // 2) 아두이노(Uno)에게 줄바꿈을 붙여서 전달
    Serial2.println(cmdFromPC);
  }

  // 3. [아두이노 -> ESP32 -> PC] 피드백 배달
  if (Serial2.available()) {
    String feedbackFromUno = Serial2.readStringUntil('\n');
    // 1) PC(소켓)로 전달
    client.println(feedbackFromUno);

    // 2) 시리얼 모니터에 출력
    Serial.print("👈 [FROM UNO]: ");
    Serial.println(feedbackFromUno);
  }
}