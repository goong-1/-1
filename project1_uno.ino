...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    data.trim(); // 혹시 모를 공백 제거

    // 🌟 [속도 측정용 로직 추가] 파이썬이 ping을 보내면 즉각 pong으로 대답
    //if (data == "ping") {
      //Serial.println("pong");
      //return; // 모터 제어 로직을 타지 않고 바로 다음 루프로 넘어감
    //}
    
    // 2. 식별자 'a', ',', 'd'의 위치를 각각 찾기
    int aIdx = data.indexOf('a');
    int commaIdx = data.indexOf(',');
    int dIdx = data.indexOf('d');
    
    // 3. 필수 구분자가 모두 들어있는지 확인
    if (aIdx != -1 && commaIdx != -1 && dIdx != -1) {
      
      // 🌟 [핵심] substring 범위를 쉼표 기준으로 조절
      // a 바로 다음부터 쉼표 직전까지 (좌측 모터)
      int leftSpeed = data.substring(aIdx + 1, commaIdx).toInt();
      
      // d 바로 다음부터 문자열 끝까지 (우측 모터)
      int rightSpeed = data.substring(dIdx + 1).toInt();
      
      // 4. 추출된 숫자로 모터 구동
      driveMotors(leftSpeed, rightSpeed);
      
      // 시리얼 모니터 확인 (쉼표가 찍히니 훨씬 보기 편할 거예요!)
      Serial.print("Parsed -> L:"); Serial.print(leftSpeed);
