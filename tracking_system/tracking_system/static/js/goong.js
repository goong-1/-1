...
window.socket = window.socket || io("http://127.0.0.1:5001", {
    transports: ["polling"]});
...
const syncCalibration = (robotId) => {
    ...
    window.socket.emit('update_calibration', { id: robotId, factor: factor });
};
...
window.moveRobot = (direction) => {
    ...
    if (key) {
        ...
        window.socket.emit('drive_control', { id: MANUAL_ROBOT_ID, command: key + '\n' });
...
function sendEmergencyCmd(command) {
    ...
    window.socket.emit('emergency_control_robot1', { 
        command: command,
        pwm: pwmVal
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.socket.on('vision_data', (data) => {
    ...
    window.socket.on('log', (data) => addLog(data.type, data.msg, data.status === 'success' ? 'info' : 'error'));
});
